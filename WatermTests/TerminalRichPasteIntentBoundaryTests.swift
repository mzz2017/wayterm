...
struct TerminalRichPasteIntentBoundaryTests {
    ...
    func applicationLayerOwnsExclusiveLeaseUseAndRemotePathInput() throws {
        ...
        let root = try sourceRoot()
        let source = try source(
            at: root.appendingPathComponent("Waterm/Features/TerminalSessions/Application/TerminalRichPasteUploadRequest.swift")
        )
    ...
    func remoteClipboardTransferDoesNotStartUntrackedStaleSweepTask() throws {
        ...
        let root = try sourceRoot()
        let source = try source(
            at: root.appendingPathComponent("Waterm/Core/SSH/RemoteClipboardTransferService.swift")
        )
