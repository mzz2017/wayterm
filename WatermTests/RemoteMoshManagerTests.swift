...
struct RemoteMoshManagerTests {
    ...
    func sshClientMoshStreamTeardownIsTrackedByClientOwner() throws {
        ...
        let clientSource = try source(at: sourceRoot().appendingPathComponent("Waterm/Core/SSH/SSHClient.swift"))
        let moshRuntimeSource = try source(
            at: sourceRoot().appendingPathComponent("Waterm/Core/SSH/SSHMoshShellRuntime.swift")
        )
    ...
    func utf8LocaleExportScriptSetsUtf8LocaleVars() {
        ...
        #expect(script.contains("waterm_validate_utf8_locale"))
        #expect(script.contains("[Uu][Tt][Ff]*8"))
        #expect(script.contains("WATERM_LOCALE_CANDIDATE"))
    ...
    func moshChildStartupScriptAlsoSetsUtf8Locale() {
        ...
        #expect(script.contains("WATERM_UTF8_LOCALE"))
