...
struct CoreSyncPendingMutationBoundaryTests {
    ...
    func pendingCloudKitMutationDoesNotStoreFeatureDomainObjects() throws {
        ...
        let source = try String(
            contentsOf: root.appendingPathComponent("Waterm/Core/Sync/PendingCloudKitSync.swift"),
            encoding: .utf8
        )
    ...
    func cloudKitSyncCoordinatorDoesNotExposeFeatureDomainEnqueueAPI() throws {
        let root = try sourceRoot()
        let source = try String(
            contentsOf: root.appendingPathComponent("Waterm/Core/Sync/CloudKitSyncCoordinator.swift"),
            encoding: .utf8
        )
    ...
    func cloudKitManagerDoesNotExposeFeatureDomainSyncAPI() throws {
        let root = try sourceRoot()
        let source = try String(
            contentsOf: root.appendingPathComponent("Waterm/Core/Sync/CloudKitManager.swift"),
            encoding: .utf8
        )
