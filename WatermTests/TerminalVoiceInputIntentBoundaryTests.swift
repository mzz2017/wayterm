...
struct TerminalVoiceInputIntentBoundaryTests {
    ...
    func voiceOverlayDelegatesSendAndCancelToApplicationStore() throws {
        let root = try sourceRoot()
        let source = try source(
            at: root.appendingPathComponent("Waterm/Features/TerminalSessions/UI/Terminal/VoiceRecordingView.swift")
        )
    ...
    private func sourceRoot() throws -> URL {
        ...
        while url.pathComponents.count > 1 {
            if FileManager.default.fileExists(atPath: url.appendingPathComponent("Waterm.xcodeproj").path) {
                return url
            }
