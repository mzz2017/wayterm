...
struct RemoteFileBrowserStoreBoundaryTests {
    ...
    func browserStoreMainFileDoesNotOwnDirectoryLoadingImplementation() throws {
        ...
        let directorySource = try source(
            at: root.appendingPathComponent("Waterm/Features/RemoteFiles/Application/RemoteFileBrowserStore+DirectoryLoading.swift")
        )
        let navigationSource = try source(
            at: root.appendingPathComponent("Waterm/Features/RemoteFiles/Application/RemoteFileNavigationCoordinator.swift")
        )
    ...
    func tabManagerUsesSnapshotStoreForPersistenceCodec() throws {
        ...
        let managerSource = try source(
            at: root.appendingPathComponent("Waterm/Features/RemoteFiles/Application/RemoteFileTabManager.swift")
        )
        let storeSource = try source(
            at: root.appendingPathComponent("Waterm/Features/RemoteFiles/Infrastructure/RemoteFileTabsSnapshotStore.swift")
        )
    ...
    func browserStoreUsesInfrastructureForPersistedStateCodec() throws {
        ...
        let persistenceSource = try source(
            at: root.appendingPathComponent("Waterm/Features/RemoteFiles/Application/RemoteFilePersistence.swift")
        )
        let persistedStateStoreSource = try source(
            at: root.appendingPathComponent("Waterm/Features/RemoteFiles/Infrastructure/RemoteFileBrowserPersistedStateStore.swift")
        )
