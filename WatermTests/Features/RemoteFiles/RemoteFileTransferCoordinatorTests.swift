...
struct RemoteFileTransferCoordinatorTests {
    ...
    func conflictResolutionComputedStateRemainsNonisolatedForTransferPlanning() throws {
        ...
        let conflictSource = try source(
            at: root.appendingPathComponent("Waterm/Features/RemoteFiles/Domain/RemoteFileConflictPolicy.swift")
        )
    ...
    func recursiveTransferCoordinatorRemainsOffMainActorExceptProgressCallback() throws {
        ...
        let recursiveTransferSource = try source(
            at: root.appendingPathComponent(
                "Waterm/Features/RemoteFiles/Application/RemoteFileRecursiveTransferCoordinator.swift"
            )
        )
        let atomicDirectoryCopySource = try source(
            at: root.appendingPathComponent(
                "Waterm/Features/RemoteFiles/Application/RemoteFileAtomicDirectoryCopyCoordinator.swift"
            )
        )
        let transferCoordinatorSource = try source(
            at: root.appendingPathComponent(
                "Waterm/Features/RemoteFiles/Application/RemoteFileTransferCoordinator.swift"
            )
        )
    ...
    func deleteDirectoryRecursivelyRemovesNestedContentsBeforeParent() async throws {
        ...
        let service = RecordingRemoteFileService(
            directoryContents: [
                "/root/.waterm": [
                    makeEntry(name: "cache", path: "/root/.waterm/cache", type: .directory),
                    makeEntry(name: "config.json", path: "/root/.waterm/config.json", type: .file),
                    makeEntry(name: "current", path: "/root/.waterm/current", type: .symlink)
                ],
                "/root/.waterm/cache": [
                    makeEntry(name: "index.db", path: "/root/.waterm/cache/index.db", type: .file)
                ]
            ]
        )

        try await store.deleteDirectoryRecursively(at: "/root/.waterm", using: service)

        #expect(service.operations == [
            .deleteFile("/root/.waterm/cache/index.db"),
            .deleteDirectory("/root/.waterm/cache"),
            .deleteFile("/root/.waterm/config.json"),
            .deleteFile("/root/.waterm/current"),
            .deleteDirectory("/root/.waterm")
        ])
    }
    ...
    func deletionCoordinatorRemovesNestedContentsBeforeParent() async throws {
        ...
        try await coordinator.deleteDirectoryRecursively(at: "/root/.waterm", using: service)
    ...
    func copyFileUploadsDestinationAtomically() async throws {
        ...
        #expect(uploadedPath.hasPrefix("/destination/.file.txt.waterm-upload-"))
    ...
    func copyEntriesResolvesDirectoryConflictBeforeAtomicRename() async throws {
        ...
        let stagingPath = try #require(createdPaths.first)
        #expect(stagingPath.hasPrefix("/destination/.folder 2.waterm-copy-"))
    ...
    func uploadFileCancellationDoesNotWriteFinalRemotePath() async throws {
        ...
        #expect(uploadedPath.hasPrefix("/remote/.report.txt.waterm-upload-"))
        #expect(operations.contains(.deleteFile(uploadedPath)))
    ...
    func uploadDirectoryCancellationDoesNotExposePartialDestinationDirectory() async throws {
        ...
        #expect(stagingPath.hasPrefix("/remote/.folder.waterm-upload-"))
        #expect(operations.contains(.deleteDirectory(stagingPath)))
