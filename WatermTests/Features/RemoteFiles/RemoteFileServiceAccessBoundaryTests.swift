...
struct RemoteFileServiceAccessBoundaryTests {
    ...
    func coreSSHDoesNotExposeRemoteFilesDomainTypes() throws {
        ...
        let sshDirectory = root.appendingPathComponent("Waterm/Core/SSH")
    ...
    func browserStoreDoesNotOwnPendingDisconnectGate() throws {
        ...
        let coordinatorSource = try source(
            at: root.appendingPathComponent("Waterm/Features/RemoteFiles/Application/RemoteFileServiceAccessCoordinator.swift")
        )
        let serviceAccessSource = try source(
            at: root.appendingPathComponent("Waterm/Features/RemoteFiles/Application/RemoteFileServiceAccessing.swift")
        )
        let adapterSource = try source(
            at: root.appendingPathComponent("Waterm/Features/RemoteFiles/Infrastructure/SSHSFTPAdapter.swift")
        )
