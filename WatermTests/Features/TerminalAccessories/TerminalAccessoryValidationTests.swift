    ...
    func testValidationErrorsRemainNonisolatedDomainDiagnostics() throws {
        let validationSource = try String(
            contentsOf: sourceRoot().appendingPathComponent(
                "Waterm/Features/TerminalAccessories/Domain/TerminalAccessoryValidation.swift"
            ),
            encoding: .utf8
        )
        let profileSource = try String(
            contentsOf: sourceRoot().appendingPathComponent(
                "Waterm/Features/TerminalAccessories/Domain/TerminalAccessoryProfileModels.swift"
            ),
            encoding: .utf8
        )
        let freeTierSource = try String(
            contentsOf: sourceRoot().appendingPathComponent(
                "Waterm/Features/Store/Domain/FreeTierLimits.swift"
            ),
            encoding: .utf8
        )
