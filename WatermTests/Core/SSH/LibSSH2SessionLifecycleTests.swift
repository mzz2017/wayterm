...
final class LibSSH2SessionLifecycleTests: XCTestCase {
    ...
    func testExecUploadNonZeroExitDoesNotCloseFreedChannelTwice() async throws {
        ...
        try await session.connect()
        do {
            try await session.upload(payload, to: "/tmp/waterm-upload", strategy: SSHUploadStrategy.execPreferred)
    ...
    func testSCPUploadChannelOpenFailurePreservesRawLibSSH2Error() async throws {
        ...
        try await session.connect()
        do {
            try await session.upload(payload, to: "/tmp/waterm-upload", strategy: SSHUploadStrategy.scpOnly)
    ...
    func testSFTPDownloadFailurePreservesExistingLocalFile() async throws {
        ...
        let localDirectory = FileManager.default.temporaryDirectory
            .appendingPathComponent("waterm-download-\(UUID().uuidString)", isDirectory: true)
    ...
    func testSFTPDownloadPublishFailureRemovesTemporaryFile() async throws {
        ...
        let remainingTemporaryDownloads = try FileManager.default.contentsOfDirectory(
            ...
            url.lastPathComponent.hasPrefix(".report.txt.waterm-download-")
        }
        XCTAssertTrue(
    ...
    private func sshSessionSource() throws -> String {
        try String(
            contentsOf: sourceRoot().appendingPathComponent("Waterm/Core/SSH/SSHSession.swift"),
            encoding: .utf8
        )
    }
    ...
    private func sshSessionChannelsSource() throws -> String {
        try String(
            contentsOf: sourceRoot().appendingPathComponent("Waterm/Core/SSH/SSHSession+Channels.swift"),
            encoding: .utf8
        )
    }
    ...
    private func sshChannelCleanupTaskRegistrySource() throws -> String {
        try String(
            contentsOf: sourceRoot().appendingPathComponent("Waterm/Core/SSH/SSHChannelCleanupTaskRegistry.swift"),
            encoding: .utf8
        )
    }
