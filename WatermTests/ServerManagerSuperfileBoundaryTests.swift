...
struct ServerManagerSuperfileBoundaryTests {
    ...
    func localPersistenceAndBootstrapLiveOutsideServerManagerFile() throws {
        ...
        let persistenceSource = try source(
            at: root.appendingPathComponent("Waterm/Features/Servers/Application/ServerManager+Persistence.swift")
        )
        let storeSource = try source(
            at: root.appendingPathComponent("Waterm/Features/Servers/Application/ServerLocalDataStore.swift")
        )
        let userDefaultsStoreSource = try source(
            at: root.appendingPathComponent("Waterm/Features/Servers/Infrastructure/UserDefaultsServerLocalDataStore.swift")
        )
        let liveSyncSource = try source(
            at: root.appendingPathComponent("Waterm/Features/Servers/Infrastructure/ServerManager+LiveSync.swift")
        )
    ...
    func cloudKitSyncAndStartupLoadingLiveOutsideServerManagerFile() throws {
        ...
        let syncSource = try source(
            at: root.appendingPathComponent("Waterm/Features/Servers/Application/ServerManager+Sync.swift")
        )
        let dependencySource = try source(
            at: root.appendingPathComponent("Waterm/Features/Servers/Application/ServerManagerSyncDependencies.swift")
        )
        let liveSyncSource = try source(
            at: root.appendingPathComponent("Waterm/Features/Servers/Infrastructure/ServerManager+LiveSync.swift")
        )
        let syncStateServiceSource = try source(
            at: root.appendingPathComponent("Waterm/Features/Servers/Application/ServerSyncStateService.swift")
        )
    ...
    func requestCoordinationLivesOutsideServerManagerFile() throws {
        ...
        let requestSource = try source(
            at: root.appendingPathComponent("Waterm/Features/Servers/Application/ServerManager+Requests.swift")
        )
    ...
    func credentialAndDeletionDefaultsLiveOutsideServerManagerFile() throws {
        ...
        let credentialSource = try source(
            at: root.appendingPathComponent("Waterm/Features/Servers/Infrastructure/ServerManager+CredentialLifecycle.swift")
        )
        let persistenceSource = try source(
            at: root.appendingPathComponent("Waterm/Features/Servers/Application/ServerCredentialPersistence.swift")
        )
        let persistenceKeychainSource = try source(
            at: root.appendingPathComponent("Waterm/Features/Servers/Infrastructure/ServerCredentialPersistence+Keychain.swift")
        )
    ...
    func deletionMutationLifecycleLivesOutsideServerManagerFile() throws {
        ...
        let deletionSource = try source(
            at: root.appendingPathComponent("Waterm/Features/Servers/Application/ServerManager+Deletion.swift")
        )
    ...
    func knownHostRemovalUsesInjectedServiceBoundary() throws {
        ...
        let serviceSource = try source(
            at: root.appendingPathComponent("Waterm/Features/Servers/Application/ServerKnownHostRemovalService.swift")
        )
        let infrastructureSource = try source(
            at: root.appendingPathComponent("Waterm/Features/Servers/Infrastructure/ServerKnownHostRemovalService+KnownHosts.swift")
        )
        let lifecycleSource = try source(
            at: root.appendingPathComponent("Waterm/Features/Servers/Infrastructure/ServerManager+KnownHostLifecycle.swift")
        )
    ...
    func queryHelpersLiveOutsideServerManagerFile() throws {
        ...
        let querySource = try source(
            at: root.appendingPathComponent("Waterm/Features/Servers/Application/ServerManager+Queries.swift")
        )
    ...
    func accessPolicyHelpersLiveOutsideServerManagerFile() throws {
        ...
        let accessSource = try source(
            at: root.appendingPathComponent("Waterm/Features/Servers/Application/ServerManager+AccessPolicy.swift")
        )
    ...
    func moveAndEnvironmentHelpersLiveOutsideServerManagerFile() throws {
        ...
        let moveSource = try source(
            at: root.appendingPathComponent("Waterm/Features/Servers/Application/ServerManager+Move.swift")
        )
        let environmentSource = try source(
            at: root.appendingPathComponent("Waterm/Features/Servers/Application/ServerManager+Environment.swift")
        )
