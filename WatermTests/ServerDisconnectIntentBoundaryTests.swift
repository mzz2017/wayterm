...
struct ServerDisconnectIntentBoundaryTests {
    ...
    func iosActiveConnectionDisconnectUsesApplicationCoordinator() throws {
        ...
        let source = try source(at: root.appendingPathComponent("Waterm/Features/Servers/UI/iOS/iOSServerListView.swift"))
    ...
    func iosCurrentServerDisconnectUsesApplicationCoordinator() throws {
        let root = try sourceRoot()
        let source = try source(at: root.appendingPathComponent("Waterm/Features/TerminalSessions/UI/iOS/iOSTerminalView.swift"))
        let helper = try slice(
    ...
    func appRootsInjectSharedDisconnectCoordinatorIntoFeatureUI() throws {
        ...
        #expect(
            ...
            "WatermApp should be the composition boundary that resolves the shared disconnect coordinator."
        )
        #expect(
            appSource.contains("disconnectTerminals:"),
            "WatermApp should configure terminal teardown on the App/Application disconnect coordinator."
        )
        #expect(
            appSource.contains("ConnectionSessionManager.shared.disconnectServerAndWait(serverId)"),
            "WatermApp should route server disconnect through the connection-session owner."
        )
        #expect(
            appSource.contains("TerminalTabManager.shared.disconnectServerAndWait(serverId)"),
            "WatermApp should route server disconnect through the terminal-tab owner."
        )
        #expect(
