...
struct IOSActiveConnectionOpenIntentBoundaryTests {
    ...
    func iosActiveConnectionOpenUsesInjectedViewTabConfiguration() throws {
        let root = try sourceRoot()
        let listSource = try source(at: root.appendingPathComponent("Waterm/Features/Servers/UI/iOS/iOSServerListView.swift"))
        let appSource = try source(at: root.appendingPathComponent("Waterm/App/iOS/iOSContentView.swift"))
