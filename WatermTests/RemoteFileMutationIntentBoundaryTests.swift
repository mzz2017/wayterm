...
struct RemoteFileMutationIntentBoundaryTests {
    ...
    func previewTextSaveDelegatesTaskOwnershipToStore() throws {
        ...
        let previewSource = try source(
            at: root.appendingPathComponent("Waterm/Features/RemoteFiles/UI/Preview/RemoteFilePreviewViews.swift")
        )
    ...
    func transferAndDropDelegatesTaskOwnershipToStore() throws {
        ...
        let transferSource = try [
            "Waterm/Features/RemoteFiles/UI/RemoteFileBrowserScreen+TransferStatus.swift",
            "Waterm/Features/RemoteFiles/UI/RemoteFileBrowserScreen+FileTransfers.swift"
        ].map { path in
            try source(at: root.appendingPathComponent(path))
        }.joined(separator: "\n")
        let macOSScreenSource = try source(
            at: root.appendingPathComponent("Waterm/Features/RemoteFiles/UI/RemoteFileBrowserMacScreen.swift")
        )
