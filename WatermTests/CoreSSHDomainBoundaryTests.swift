...
struct CoreSSHDomainBoundaryTests {
    ...
    func coreSSHDoesNotExposeServerDomainConnectionAPI() throws {
        ...
        let coreDirectories = [
            root.appendingPathComponent("Waterm/Core/SSH"),
            root.appendingPathComponent("Waterm/Core/Network/Cloudflare")
        ]
        let forbiddenShapes = [
            "Server?",
            "Server,",
    ...
    func sshConnectionPrimitivesAreOwnedByCoreSSH() throws {
        let root = try sourceRoot()
        let serverSource = try source(at: root.appendingPathComponent("Waterm/Features/Servers/Domain/Server.swift"))
        let targetSource = try source(at: root.appendingPathComponent("Waterm/Core/SSH/SSHConnectionTarget.swift"))
