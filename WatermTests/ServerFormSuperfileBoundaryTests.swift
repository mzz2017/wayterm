...
struct ServerFormSuperfileBoundaryTests {
    ...
    func serverFormSheetDoesNotOwnMoveServerSheet() throws {
        ...
        let moveSource = try source(
            at: root.appendingPathComponent("Waterm/Features/Servers/UI/ServerDetail/MoveServerSheet.swift")
        )
    ...
    func serverFormSheetDoesNotOwnTransportSelectionOrCredentialBuilder() throws {
        ...
        let selectionSource = try source(
            at: root.appendingPathComponent("Waterm/Features/Servers/UI/ServerDetail/ServerTransportSelection.swift")
        )
        let credentialBuilderSource = try source(
            at: root.appendingPathComponent("Waterm/Features/Servers/Application/ServerFormCredentialBuilder.swift")
        )
        let submissionBuilderSource = try source(
            at: root.appendingPathComponent("Waterm/Features/Servers/Application/ServerFormSubmissionBuilder.swift")
        )
        let formDefaultsSource = try source(
            at: root.appendingPathComponent("Waterm/Features/Servers/Infrastructure/ServerFormDefaults.swift")
        )
        let validationPolicySource = try source(
            at: root.appendingPathComponent("Waterm/Features/Servers/Application/ServerFormValidationPolicy.swift")
        )
    ...
    func serverFormSheetComposesFieldSectionsWithoutOwningTheirLayout() throws {
        ...
        let sectionSource = try source(
            at: root.appendingPathComponent("Waterm/Features/Servers/UI/ServerDetail/ServerFormSections.swift")
        )
        let platformModifierSource = try source(
            at: root.appendingPathComponent("Waterm/Features/Servers/UI/ServerDetail/ServerFormPlatformModifiers.swift")
        )
    ...
    func serverFormsDoNotResolveSharedServerManager() throws {
        let root = try sourceRoot()
        let formSources = try [
            "Waterm/Features/Servers/UI/ServerDetail/ServerFormSheet.swift",
            "Waterm/Features/Servers/UI/Workspace/EnvironmentFormSheet.swift",
            "Waterm/Features/Servers/UI/Workspace/WorkspaceFormSheet.swift"
        ].map { path in
    ...
    func workspaceFormUsesInjectedStoreManager() throws {
        ...
        let workspaceFormSource = try source(
            at: root.appendingPathComponent("Waterm/Features/Servers/UI/Workspace/WorkspaceFormSheet.swift")
        )
        let callerSources = try [
            "Waterm/Features/Servers/UI/ServerDetail/MoveServerSheet.swift",
            "Waterm/Features/Servers/UI/ServerDetail/ServerFormSheet.swift",
    ...
    func serverFormUsesInjectedStoreManager() throws {
        ...
        let callerSources = try [
            ...
            "Waterm/Features/TerminalSessions/UI/iOS/IOSTerminalPresentationHost.swift"
        ].map { path in
            try source(at: root.appendingPathComponent(path))
        }.joined(separator: "\n")
