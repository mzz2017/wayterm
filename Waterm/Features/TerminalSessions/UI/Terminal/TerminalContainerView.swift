...
struct TerminalContainerView: View {
    ...
    private var retrustHostConfirmationMessage: String {
        guard let server else {
            return String(localized: "Waterm will forget the saved SSH host key and reconnect.")
        }
        let endpoint = "\(server.host):\(server.port)"
        return String(
            format: String(localized: "Waterm will trust the SSH host key presented by %@ on the next reconnect. Only continue if you recognize this server."),
            endpoint
        )
    }
