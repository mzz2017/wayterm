...
extension ServerManager {
    ...
    func moveServer(
        ...
        guard let refreshedDestination = workspace(withId: destination.id) else {
            throw WatermError.moveNotAllowed(String(localized: "The destination workspace is no longer available."))
        }
    ...
    private func moveRestriction(for server: Server, destination: Workspace) -> WatermError? {
        ...
        if !isProProvider() && isWorkspaceLocked(destination) {
            return WatermError.proRequired(String(localized: "Upgrade to Pro to move servers into locked workspaces"))
        }

        return WatermError.moveNotAllowed(String(localized: "This server can't be moved to that workspace right now."))
    }
