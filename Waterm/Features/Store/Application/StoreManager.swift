...
final class StoreManager: ObservableObject {
    ...
    var monthlyProduct: Product? {
        products.first { $0.id == WatermProducts.proMonthly }
    }

    var yearlyProduct: Product? {
        products.first { $0.id == WatermProducts.proYearly }
    }

    var lifetimeProduct: Product? {
        products.first { $0.id == WatermProducts.proLifetime }
    }
    ...
    func loadProducts() async {
        ...
        for attempt in 0..<maxRetries {
            do {
                products = try await Product.products(for: WatermProducts.allProducts)
    ...
    func checkEntitlements() async {
        ...
        for await result in Transaction.currentEntitlements {
            ...
            if case .verified(let transaction) = result {
                ...
                case WatermProducts.proMonthly,
                     WatermProducts.proYearly:
                    hasAccess = true
                case WatermProducts.proLifetime:
                    hasAccess = true
                    hasLifetime = true
                default:
                    break
