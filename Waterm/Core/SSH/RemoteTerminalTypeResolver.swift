...
enum RemoteTerminalTypeResolver {
    ...
    private static let logger = Logger(
        subsystem: Bundle.main.bundleIdentifier ?? "Waterm",
    ...
    nonisolated private static let probeMarker = "__WATERM_XTERM_GHOSTTY_OK__"
    nonisolated private static let probeMissMarker = "__WATERM_XTERM_GHOSTTY_NO__"
    private static let installMarker = "__WATERM_XTERM_GHOSTTY_INSTALLED__"
    private static let missingTicMarker = "__WATERM_XTERM_GHOSTTY_NO_TIC__"
    private static let installFailedMarker = "__WATERM_XTERM_GHOSTTY_INSTALL_FAILED__"
    private static let hereDocMarker = "__WATERM_XTERM_GHOSTTY_TERMINFO__"
    ...
    nonisolated static func probeCommand(okMarker: String = probeMarker) -> String {
        ...
        if { command -v infocmp >/dev/null 2>&1 && infocmp -x xterm-ghostty >/dev/null 2>&1; } || waterm_has_xterm_ghostty_terminfo; then
          printf '\(okMarker)';
        else
    ...
    nonisolated private static func hasGhosttyTerminfoFunction() -> String {
        """
        waterm_has_xterm_ghostty_terminfo() {
          WATERM_OLD_IFS=$IFS;
          IFS=:;
