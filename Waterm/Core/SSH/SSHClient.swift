...
nonisolated actor SSHClient {
    ...
    private let logger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "Waterm", category: "SSH")
    ...
    func connect(to target: SSHConnectionTarget, credentials: ServerCredentials) async throws -> SSHSession {
        ...
        let task = Task { () -> SSHSession in
            ...
            if target.connectionMode == .cloudflare {
                ...
                await self.disconnectCloudflareTransport(reason: "pre-connect cleanup")
            }
        ...
        do {
            ...
            guard isCurrentConnectRequest(requestID),
                  isCurrentPendingConnectSession(session) else {
                session.abort()
