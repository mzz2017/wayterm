...
actor RemoteMoshManager {
    ...
    private let logger = Logger(subsystem: "app.vivy.Waterm", category: "mosh-bootstrap")
    private static let installSuccessMarker = "__WATERM_MOSH_INSTALLED__"
    ...
    func isMoshServerAvailable(using executor: any RemoteCommandExecuting) async -> Bool {
        let okMarker = "__WATERM_MOSH_OK__"
        let body = "\(RemoteTerminalBootstrap.shellPathExport()); if command -v mosh-server >/dev/null 2>&1; then printf '\(okMarker)'; else printf '__WATERM_MOSH_NO__'; fi"
    ...
    nonisolated func installScript() -> String {
        ...
        if command -v mosh-server >/dev/null 2>&1; then printf '\(Self.installSuccessMarker)'; else printf '__WATERM_MOSH_INSTALL_FAILED__'; fi
        """
    }
    ...
    nonisolated func utf8LocaleExportScript() -> String {
        """
        waterm_validate_utf8_locale() {
          [ -n "$1" ] || return 1;
          WATERM_TEST_CHARMAP="$(LANG="$1" LC_ALL="$1" LC_CTYPE="$1" locale charmap 2>/dev/null)" || return 1;
          case "$WATERM_TEST_CHARMAP" in *[Uu][Tt][Ff]*8*) return 0 ;; *) return 1 ;; esac
        };
        WATERM_CURRENT_CHARMAP="$(locale charmap 2>/dev/null || true)";
        case "$WATERM_CURRENT_CHARMAP" in
          *[Uu][Tt][Ff]*8*) WATERM_UTF8_LOCALE="" ;;
          *)
            WATERM_UTF8_LOCALE="";
            for WATERM_LOCALE_CANDIDATE in "${LC_ALL:-}" "${LC_CTYPE:-}" "${LANG:-}" C.UTF-8 C.utf8 en_US.UTF-8 en_US.utf8; do
              if waterm_validate_utf8_locale "$WATERM_LOCALE_CANDIDATE"; then
                WATERM_UTF8_LOCALE="$WATERM_LOCALE_CANDIDATE";
                break;
              fi;
            done;
            if [ -z "$WATERM_UTF8_LOCALE" ] && command -v locale >/dev/null 2>&1; then
              for WATERM_LOCALE_CANDIDATE in $(locale -a 2>/dev/null || true); do
                case "$WATERM_LOCALE_CANDIDATE" in
                  *[Uu][Tt][Ff]*8*)
                    if waterm_validate_utf8_locale "$WATERM_LOCALE_CANDIDATE"; then
                      WATERM_UTF8_LOCALE="$WATERM_LOCALE_CANDIDATE";
                      break;
            ...
            if [ -n "$WATERM_UTF8_LOCALE" ]; then
              export LANG="$WATERM_UTF8_LOCALE";
              export LC_ALL="$WATERM_UTF8_LOCALE";
              export LC_CTYPE="$WATERM_UTF8_LOCALE";
            fi
            ;;
        esac;
        unset WATERM_CURRENT_CHARMAP WATERM_LOCALE_CANDIDATE WATERM_TEST_CHARMAP;
        unset -f waterm_validate_utf8_locale 2>/dev/null || true
        """
    }
