...
- [x] **Step 1: Add RED split surface callback boundary tests**
...
- [x] **Step 2: Pass the injected TerminalTabManager into pane views and wrappers**
...
- [x] **Step 3: Route split pane surface callbacks through the injected manager**
...
**Interfaces:**
- Consumes:
  - Task 75's injected `sessionManager` requirement on `SSHTerminalCoordinator`.
  - Existing root coordinator state: `sessionId`, `server.id`, `terminalView`, macOS `isReusingTerminal`, and iOS `preserveSession`.
  - Existing `ConnectionSessionManager` APIs: `sessions`, `detachSurfaceForViewDisappeared(from:)`, and `handleClosedSessionSurfaceTeardown(sessionId:serverId:reason:)`.
- Produces:
  - macOS `static func dismantleNSView` reads session liveness and sends detach/teardown intent through `coordinator.sessionManager`.
...
- Invariant: static teardown may pause/resign UI surfaces locally, but it must not resolve `ConnectionSessionManager.shared`; it must use `coordinator.sessionManager`.
- Fake assumptions: these are source-boundary tests because constructing `NSViewRepresentable` / `UIViewRepresentable` teardown inputs would require platform UI surfaces and Ghostty.
...
Add `iosStaticTeardownUsesCoordinatorManager`:
- Slice from `static func dismantleUIView` ending before `private func terminalHostView`.
- Assert the slice contains `coordinator.sessionManager.sessions`, `coordinator.sessionManager.detachSurfaceForViewDisappeared`, and `coordinator.sessionManager.handleClosedSessionSurfaceTeardown`.
- Assert the slice does not contain `ConnectionSessionManager.shared`.
...
- In macOS `static func dismantleNSView`, replace `ConnectionSessionManager.shared.detachSurfaceForViewDisappeared(from:)` with `coordinator.sessionManager.detachSurfaceForViewDisappeared(from:)`.
- In macOS `static func dismantleNSView`, replace `ConnectionSessionManager.shared.handleClosedSessionSurfaceTeardown(...)` with `coordinator.sessionManager.handleClosedSessionSurfaceTeardown(...)`.
...
**Interfaces:**
- Consumes:
  - Task 74's injected `tabManager` on `SSHTerminalPaneWrapper.Coordinator`.
  - Existing split coordinator state: `paneId`, `terminal`, and `isReusingTerminal`.
  - Existing `TerminalTabManager` APIs: `paneStates`, `detachSurfaceForPaneViewDisappeared(_:)`, and `detachSurfaceForClosedPane(_:)`.
- Produces:
...
- Invariant: static teardown may pause UI surfaces locally, but it must not resolve `TerminalTabManager.shared`; it must use `coordinator.tabManager`.
- Fake assumptions: these are source-boundary tests because constructing `NSViewRepresentable` teardown inputs would require platform UI surfaces and Ghostty.
...
- Replace `TerminalTabManager.shared.detachSurfaceForPaneViewDisappeared(coordinator.paneId)` with `coordinator.tabManager.detachSurfaceForPaneViewDisappeared(coordinator.paneId)`.
...
- [x] **Step 1: Add RED split UI injected-manager boundary tests**
...
- [x] **Step 2: Route split UI helpers through the injected manager**
...
**Interfaces:**
- Consumes:
  - Task 75's root coordinator `sessionManager: ConnectionSessionManager`.
  - Task 74's split pane coordinator `tabManager: TerminalTabManager`.
  - Existing `ConnectionSessionManager` APIs: `requestSessionRichPasteUpload(...)` and `peekTerminal(for:)`.
  - Existing `TerminalTabManager` APIs: `requestPaneRichPasteUpload(...)` and `getTerminal(for:)`.
- Produces:
  - `TerminalRichPasteRuntime.connectionSession(sessionId:uiModel:sessionManager:)`.
  - `TerminalRichPasteRuntime.terminalPane(paneId:uiModel:tabManager:)`.
  - Root rich-paste coordinators pass the injected `sessionManager` into runtime construction on macOS and iOS.
...
- Invariant: `TerminalRichPasteRuntime` factories must not resolve manager singletons; they must receive root/split managers from the representable coordinator.
- Fake assumptions: these are source-boundary tests because the protected behavior is dependency ownership at the SwiftUI/coordinator/application boundary, while upload lifecycle ordering is covered by `TerminalRichPasteUploadRequestTests`.
...
- Slice from `static func terminalPane` ending before `func install`.
- Assert the pane slice contains `tabManager: TerminalTabManager`, `tabManager.requestPaneRichPasteUpload`, and `tabManager.getTerminal`.
- Assert the pane slice does not contain `TerminalTabManager.shared`.
...
- Replace `ConnectionSessionManager.shared.requestSessionRichPasteUpload(...)` with `sessionManager.requestSessionRichPasteUpload(...)`.
- Replace `ConnectionSessionManager.shared.peekTerminal(for:)` with `sessionManager.peekTerminal(for:)`.
- Change `terminalPane(paneId:uiModel:)` to `terminalPane(paneId:uiModel:tabManager:)`.
- Replace `TerminalTabManager.shared.requestPaneRichPasteUpload(...)` with `tabManager.requestPaneRichPasteUpload(...)`.
...
- [x] **Step 1: Add RED foreground reconnect request tracking tests**
...
- `duplicateForegroundReconnectRequestsPreserveEachCallbackAction`
  - Call the request API twice for the same selected session with different `refreshTerminal` presentation values.
  - Assert reconnect runs under one request ID, but each callback receives the action computed for its own intent.
...
- [x] **Step 2: Add manager-owned foreground reconnect request API**
...
- Add `requestForegroundReconnectForSelectedSession(...) -> UUID?` that:
  ...
  - Calls each callback with the presentation action computed for the intent that registered that callback.
- Cancel foreground reconnect requests from session close/reset paths without surfacing cancellation as failure.

- [x] **Step 3: Route iOS foreground helper through request intent**
...
- 2026-06-21: Task 80 RED/GREEN completed with local lifecycle review and no new subagents. `ConnectionSessionManager` now owns tracked iOS foreground reconnect requests through `requestForegroundReconnectForSelectedSession(...)`, `pendingForegroundReconnectRequestIDs`, and `waitForForegroundReconnectRequest(_:)`; duplicate selected-session requests coalesce reconnect work while preserving each callback's own presentation action, and close/reset paths cancel visible pending state while keeping started work awaitable until blocked reconnect exits. `iOSContentView.attemptForegroundReconnectIfNeeded(refreshTerminal:)` now sends synchronous intent to the manager and updates only presentation state in the callback; it no longer starts a local reconnect `Task` or calls `handleForegroundReconnectForSelectedSession(...)`. Initial RED failed as expected because the request API/testing seam did not exist and the iOS helper still owned a local task. API cleanup found a stale callback-action edge case; `duplicateForegroundReconnectRequestsPreserveEachCallbackAction` reproduced it red, then GREEN passed after storing callback/action pairs. Final verification passed 101 Swift Testing tests across `ConnectionLifecycleIntegrationTests` and `IOSForegroundReconnectIntentBoundaryTests`. Source scan showed the iOS helper calls `requestForegroundReconnectForSelectedSession(...)` and no helper-local reconnect task remains. `git diff --check` passed; iOS `build-for-testing` passed with `ENABLE_DEBUG_DYLIB=NO`. API/boundary cleanup found request API naming consistent with existing manager-owned request APIs, UI limited to presentation updates, no temporary helper/WIP state, no new stale selected-session write, and no new SwiftUI-owned lifecycle orchestration.
