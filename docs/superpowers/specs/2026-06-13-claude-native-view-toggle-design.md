...
Date: 2026-06-13
Branch (Waterm): TBD（建议 `feat/claude-native-view`）
依赖：`feat/zmx-session-binding-ssh-auth`（已合入 main，提供 zmx 多路复用 + 会话绑定）

## Summary

Waterm 已支持通过 zmx（轻量级终端多路复用器）保持远端会话持久化。当用户在一个
...
- **终端透传型**（waterm 现状）：agent loop 是**真 Claude Code**，永远最新——代价是被迫
...
- 完全复用 waterm 现有的 SSH 传输、Keychain、zmx 会话绑定。
...
其中 `<cwd-slug>` 是工作目录路径的转义形式（如 `-home-mzz-projects-waterm`），
...
- 在 zmx 里启动 claude 时由 waterm 注入固定 uuid：`claude --session-id $UUID`；
...
**含义**：除了 tail jsonl，waterm 还有第二条独立的「旁路观察」通道可选（VT 字节级），
...
1. **会话真相**：zmx + claude，照常跑，waterm 不改其行为，只在启动时注入 `--session-id`。
...
以下接缝来自对 waterm 当前代码的实地勘查。注意分支差异：
...
waterm 已有「视图切换器」基础设施 `ConnectionViewTab`（终端 / 文件浏览 等分段）。
...
2. 存在与之绑定的 `claudeSessionId`（说明是 waterm 通过 `--session-id` 启动的 claude）；
   或
3. 兜底启发式：远端 `~/.claude/projects/<cwd-slug>/` 下存在 mtime 活跃、cwd 匹配的
   jsonl（适配「用户在 zmx 里自己手敲 `claude`」的情况——此时无注入 id，需按 cwd+mtime
   猜测，歧义风险见 §开放问题）。

最稳的是路径 2：由 waterm 主动用 `--session-id` 启动 claude，从源头消除歧义。
...
3. **本方案的独特点**：用 **zmx 持久化** 把「同一活会话」做实，使「原始 ⇄ 原生」成为
   *两个旁路读者之间的切换*，而非*会话的重启/接管*。这是 waterm 现有资产（zmx 绑定 +
...
| 「用户手敲 claude」无注入 id | transcript 定位靠 cwd+mtime 猜测，可能歧义 | 优先由 waterm 用 `--session-id` 启动；手敲场景做 best-effort + 让用户在多候选时确认 |
...
3. **手敲 claude 的会话发现**：cwd+mtime 启发式在多会话同 cwd 时如何消歧？是否需要在
   zmx 层暴露「当前前台进程 + 其 --session-id」给 waterm 探测？
