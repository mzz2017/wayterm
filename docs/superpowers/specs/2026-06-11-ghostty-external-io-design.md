# Switch VVTerm to ghostty-zmx External I/O backend

Date: 2026-06-11
Branch (VVTerm): `feat/ghostty-external-io`
Fork target: `mzz2017/ghostty` branch `ios-external-io`

## Goal

Migrate VVTerm's SSH↔terminal bridge from the `wiedymi/ghostty` `custom-io`
...
The new API reuses VVTerm's EXISTING `surfaceConfig.userdata`
...
- `VVTerm/GhosttyTerminal/GhosttyRenderingSetup.swift`: replace
  ...
  backend choice.
- `VVTerm/GhosttyTerminal/Ghostty.Surface.swift`: replace `feedData` body to
  call `ghostty_surface_write_output`; drop `setWriteCallback`/`WriteCallback`
  (now config-time); add `externalExited(code:)` wrapper.
- `VVTerm/GhosttyTerminal/GhosttyTerminalView+macOS.swift` &
...
- `VVTerm/Features/TerminalSessions/UI/Terminal/SSHTerminalWrapper.swift`
  (macOS + iOS) & `VVTerm/Features/TerminalSessions/UI/Splits/TerminalView.swift`
...
VVTerm's vendored `ghostty.h` is regenerated by layer B (build.sh rsyncs the
