---
name: waterm-commit-message
description: Use when drafting, reviewing, amending, or committing Waterm git commit messages. Trigger for git commit, commit title, commit body, conventional commit, reword, squash, changelog-oriented history, or when deciding whether a staged diff should be split into atomic commits.
---

# Waterm Commit Message

Use this skill to write Waterm commits that are conventional, searchable, and useful during review. Preserve the repo's existing `fix:`/`test:`/`ci:` style, but make the subject carry enough information to understand the affected owner and the durable change.
...
Validated with WatermTests/SSHClientSupportOwnerTests.
```
...
Use `git commit -m` only for allowed subject-only commits. For normal Waterm
