...
struct ServerManagerSharedTypeBoundaryTests {
    ...
    func serverManagerDoesNotOwnSharedLimitAndErrorTypes() throws {
        ...
        let limitsSource = try source(
            at: root.appendingPathComponent("VVTerm/Features/Store/Domain/FreeTierLimits.swift")
        )
        let errorSource = try source(
            at: root.appendingPathComponent("VVTerm/Core/VVTermError.swift")
        )
        ...
        #expect(
            !managerSource.contains("enum VVTermError"),
        ...
        #expect(errorSource.contains("enum VVTermError"))
    }
