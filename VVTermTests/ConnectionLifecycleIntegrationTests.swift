...
struct ConnectionLifecycleIntegrationTests {
    ...
    func connectionManagerCloseSessionCancelsPendingInstallRequestsAndFinishesMoshCleanup() async {
        await withCleanConnectionManager { manager in
            let session = ConnectionSession(serverId: UUID(), title: "Session")
            let tmuxGate = InstallOperationGate()
            let moshGate = InstallOperationGate()
            manager.sessions = [session]
    ...
    func connectionManagerRecordsMoshInstallFailureWithoutCallingSuccess() async {
        await withCleanConnectionManager { manager in
            ...
            #expect(completedCount == 0)
            #expect(failedErrors == [TestInstallError.failed.localizedDescription])
            #expect(
                manager.lastMoshInstallFailure?.localizedDescription == TestInstallError.failed.localizedDescription,
...
private actor InstallOperationGate {
    private var releaseContinuation: CheckedContinuation<Void, Never>?
