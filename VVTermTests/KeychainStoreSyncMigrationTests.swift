...
// Cloudflare token persistence. When VVTerm sync is disabled, rewriting a
...
struct KeychainStoreSyncMigrationTests {
    ...
    func rewritingSyncedItemAsDeviceLocalDeletesSyncedVariant() throws {
        ...
        let store = KeychainStore(service: "app.vivy.vvterm.tests", secItemClient: client)
        try client.seed(
            service: "app.vivy.vvterm.tests",
